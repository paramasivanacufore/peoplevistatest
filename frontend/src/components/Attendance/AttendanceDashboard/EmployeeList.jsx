...
import { getEmployeesWithAttendance, getDepartments, exportEmployeesWithAttendance } from '../../../utils/attendance/apiUtils';
import { getShifts } from '../../../utils/attendance/apiUtils';
...
const EmployeesList = () => {
    ...
    }, [currentPage, itemsPerPage, searchTerm, selectedDate, selectedDepartments, selectedShifts, selectedStatus, sortColumn, sortDirection, filtersSubmitted]);
    ...
    const fetchEmployees = async () => {
        try {
            ...
            const data = await getEmployeesWithAttendance(params);
    ...
    const exportToCSV = async () => {
        try {
            ...
            const data = await exportEmployeesWithAttendance(params);
            ...
            const headers = ['Employee ID', 'Employee Name', 'Department', 'Shift', 'Date', 'Status', 'In Time', 'Out Time', 'Working Hours'];
            ...
            const filename = `employees_attendance_${dateStr}.csv`;
    ...
    const getStatusBadge = (status) => {
        const statusConfig = {
            'Present': { bg: 'bg-green-100', text: 'text-green-800', icon: FiCheckCircle },
            'Absent': { bg: 'bg-red-100', text: 'text-red-800', icon: FiXCircle },
            'Leave': { bg: 'bg-orange-100', text: 'text-orange-800', icon: FiAlertCircle },
            'Holiday': { bg: 'bg-blue-100', text: 'text-blue-800', icon: FiCalendar },
            'Week Off': { bg: 'bg-gray-100', text: 'text-gray-800', icon: FiCalendar },
            'Regularized': { bg: 'bg-purple-100', text: 'text-purple-800', icon: FiCheckCircle }
        };
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-3 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-auto lg:w-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 flex-1">
                        ...
                        <div className="relative min-w-[150px] sm:min-w-[160px] flex-1 sm:flex-none">
                        <button
                            onClick={() => {
                        ...
                        <div className="relative flex-1 sm:flex-none">
                            <FiCalendar className="absolute left-2.5 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4 pointer-events-none" />
                            <input
                                type="date"
                                value={selectedDate}
                                onChange={(e) => {
                                ...
                                className="pl-8 pr-3 py-1.5 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-gray-300 w-full"
                            />
                        </div>
            ...
            {(selectedDepartments.length > 0 || selectedShifts.length > 0 || (selectedStatus && selectedStatus !== 'All Status')) && (
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-2 sm:mt-2 pt-2 sm:pt-2 border-t border-gray-200 mb-3">
                        ...
                        {selectedStatus && selectedStatus !== 'All Status' && (
                            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-100 border border-gray-300 rounded-full text-xs sm:text-sm text-gray-700">
                                <span>{selectedStatus}</span>
                                <button
            ...
            <DataTable
                ...
                renderRow={(employee) => (
                    <tr key={employee.employee_id || employee.id} className="ui-row">
                        ...
                        <td className="ui-td">{employee.status || 'N/A'}</td>
                        <td className="ui-td">
                            <div className="flex items-center gap-1">
                                <FiClock className="w-4 h-4 text-gray-400" />
                                {formatTime(employee.first_in_time)}
                            </div>
                        </td>
                        <td className="ui-td">
                            <div className="flex items-center gap-1">
                                <FiClock className="w-4 h-4 text-gray-400" />
                                {formatTime(employee.last_out_time)}
                            </div>
                        </td>
                        <td className="ui-td">{employee.total_working_hours || 'N/A'}</td>
                    </tr>
                )}
