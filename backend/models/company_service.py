...
class CompanyService:
    ...
    def create_company(company_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if company_data.get('registration_no'):
                existing = CompanyService.get_company_by_registration_no(company_data['registration_no'])
                if existing:
                    raise ValueError(f"Company with registration number '{company_data['registration_no']}' already exists")
            
            # Check if company name already exists
            existing = CompanyService.get_company_by_name(company_data['company_name'])
            if existing:
                raise ValueError(f"Company '{company_data['company_name']}' already exists")
            
            # Extract address data if provided
            address_data = {}
            if 'address_line1' in company_data or 'postal_code' in company_data:
                address_data = {
                    ...
                    'postal_code': company_data.get('postal_code') or company_data.get('zip_code'),
                    'address_type': 'Company'
                }
            
            # Prepare company insert data
            company_insert = {
            ...
            company_insert['created_at'] = current_time
            company_insert['updated_at'] = current_time
            
            company_id = db.execute_insert(query, company_insert)
            
            # Create address if provided
            if address_data.get('address_line1') and address_data.get('city'):
                CompanyService._create_address(company_id, address_data, 'Company')
    ...
    def get_company_by_id(company_id: int) -> Optional[Dict[str, Any]]:
        """Get company by ID with address"""
        try:
            query = """
                SELECT 
                    c.*,
                    ...
                    a.address_type
                FROM companies c
                LEFT JOIN addresses a ON c.company_id = a.company_id 
                    AND a.address_type = 'Company'
            ...
            # Format response
            company_data = CompanyService._format_company_response(result)
            return company_data
    ...
    def get_all_companies() -> List[Dict[str, Any]]:
        """Get all companies with status mapping"""
        try:
            query = """
                SELECT 
                    company_id,
            ...
            status_map = {1: "Active", 2: "Archived"}
            ...
            for row in results:
                companies.append({
                    ...
                    "status": status_map.get(row.get("status_id"), "Unknown")
                })
    ...
    def update_company(company_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """Update a company and its address"""
        try:
            # Check if company exists
            if not CompanyService.check_company_exists(company_id):
                raise ValueError(f"Company with ID {company_id} not found")
            
            # Check for duplicate registration_no if updating
            if 'registration_no' in update_data and update_data['registration_no']:
                existing = CompanyService.get_company_by_registration_no(update_data['registration_no'])
                if existing and existing['company_id'] != company_id:
                    raise ValueError(f"Company with registration number '{update_data['registration_no']}' already exists")
            ...
            if 'company_name' in update_data and update_data['company_name']:
                ...
                if existing and existing['company_id'] != company_id:
                    raise ValueError(f"Company '{update_data['company_name']}' already exists")
            
            # Build update query for company
            set_clauses = []
            params = []
            
            company_fields = ['company_name', 'registration_no', 'industry_type', 'website_url', 
            ...
            if has_address_data:
                ...
                existing_address = db.execute_query_one(
                    "SELECT address_id FROM addresses WHERE company_id = %s AND address_type = 'Company'",
                    (company_id,)
                )
                ...
                address_data = {
                    ...
                    'postal_code': update_data.get('postal_code') or update_data.get('zip_code'),
                    'address_type': 'Company'
                }
                
                if existing_address:
                    # Update existing address
                    CompanyService._update_address(existing_address['address_id'], address_data)
                elif address_data.get('address_line1') and address_data.get('city'):
                    # Create new address
                    CompanyService._create_address(company_id, address_data, 'Company')
